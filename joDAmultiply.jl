...
D1=joDAdistribute(dims1[2]);show(D1);show(D1')
D2=joDAdistribute(dims2[2],nvc2);show(D2);show(D2')

println("... joDAgather ...")
G1=joDAgather(dims1[2]);show(G1);show(G1')
G2=joDAgather(dims2[2],nvc2);show(G2);show(G2')
...
OR=joGaussian(M+1,N);
O=OL*OM*OR
DL=joDAdistributedLinOp(OL,NVC);
DM=joDAdistributedLinOp(OM,NVC);
DR=joDAdistributedLinOp(OR,NVC);
DO=DL*DM*DR; show(DO)
A=joDAdistributedLinOp(O,NVC); show(A)
D=joDAdistribute(size(O,2),NVC); show(D)
G=joDAgather(size(O,1),NVC); show(G)
